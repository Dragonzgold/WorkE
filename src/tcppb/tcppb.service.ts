...
export class TcppbService {
  ...
  async findAll(select: string, dateStart: Date, dateEnd: Date) {
    try {
      ...
      const dataToTransfer = await this.datatotransfer.query(`SELECT * FROM tcppb WHERE TIPODOCU LIKE '%${testwork}%' AND FECHA BETWEEN '${dateOne}' AND '${dateTwo}'`);
      ...
      const columns = await this.seconddataSource.query(`SHOW COLUMNS FROM tcppb`);
      ...
      for (const row of dataToTransfer) {
        ...
        const existingRow = await this.seconddataSource.query('SELECT 1 FROM tcppb WHERE CODIPROV = ? AND TIPODOCU = ? AND NUMEDOCU = ? AND LINENUME = ?',[codiprov, tipodocu, numedocu, linenume]);
        ...
        if (existingRow.length === 0) {
          ...
          if (foreignKeyExists.length > 0) {
            const insertQuery = `INSERT INTO tcppb (${columnNames.join(', ')}) VALUES (${columnNames.map(() => '?').join(', ')})`;
