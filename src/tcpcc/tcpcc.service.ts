...
export class TcpccService {
  ...
  async findAll(select: string, dateStart: Date, dateEnd: Date) {
    try {
      ...
      const dataToTransfer = await this.datatotransfer.query(`SELECT * FROM tcpcc WHERE TIPODOCU LIKE '%${testwork}%' AND FECHA BETWEEN '${dateOne}' AND '${dateTwo}'`);
      ...
      const columns = await this.seconddataSource.query(`SHOW COLUMNS FROM tcpcc`);
      ...
      for (const row of dataToTransfer) {
        ...
        const existingRow = await this.seconddataSource.query('SELECT 1 FROM tcpcc WHERE TIPODOCU = ? AND NUMEDOCU = ? AND LINENUME = ?',[tipodocu, numedocu, linenume]);
        ...
        if (existingRow.length === 0) {
          ...
          if (foreignKeyExists.length > 0) {
            const insertQuery = `INSERT INTO tcpcc (${columnNames.join(', ')}) VALUES (${columnNames.map(() => '?').join(', ')})`;
