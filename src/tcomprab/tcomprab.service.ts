...
export class TcomprabService {
  ...
  async findAll(select:string, dateStart:Date, dateEnd:Date) {
    try {
      ...
      const dataToTransfer = await this.datatotransfer.query(`SELECT * FROM tcomprab WHERE TIPODOCU LIKE '%${testwork}%' AND FECHA BETWEEN '${dateOne}' AND '${dateTwo}'`);
      ...
      const columns = await this.seconddataSource.query(`SHOW COLUMNS FROM tcomprab`);
      ...
      for (const row of dataToTransfer) {
        ...
        const codiprod = row['CODIPROD']
    
        const existingRow = await this.seconddataSource.query('SELECT 1 FROM tcomprab WHERE CODIPROV = ? AND TIPODOCU = ? AND NUMEDOCU = ? AND CODIPROD = ?',[codiprov, tipodocu, numedocu, codiprod]);
        ...
        if (existingRow.length === 0) {
          ...
          if (foreignKeyExists.length > 0) {
            const insertQuery = `INSERT INTO tcomprab (${columnNames.join(', ')}) VALUES (${columnNames.map(() => '?').join(', ')})`;
