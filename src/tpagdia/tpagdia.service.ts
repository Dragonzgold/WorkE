...
export class TpagdiaService {
  ...
  async findAll(select:string, dateStart:Date, dateEnd:Date) {
    try {
      ...
      const dataToTransfer = await this.datatotransfer.query(`SELECT * FROM tpagdiaa WHERE TIPODOCU LIKE '%${testwork}%' AND FECHA BETWEEN '${dateOne}' AND '${dateTwo}'`);
      ...
      const columns = await this.seconddataSource.query(`SHOW COLUMNS FROM tpagdiaa`);
      ...
      for (const row of dataToTransfer) {
        ...
        const existingRow = await this.seconddataSource.query('SELECT 1 FROM tpagdiaa WHERE NUMERECI = ?',[numereci]);
        ...
        if (existingRow.length === 0) {
          ...
          if (foreignKeyExists.length > 0) {
            const insertQuery = `INSERT INTO tpagdiaa (${columnNames.join(', ')}) VALUES (${columnNames.map(() => '?').join(', ')})`;
