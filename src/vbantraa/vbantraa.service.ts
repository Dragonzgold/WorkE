...
export class VbantraaService {
  ...
  async findAll(select:string, dateStart:Date, dateEnd:Date) {
    try {
      ...
      const dataToTransfer = await this.datatotransfer.query(`SELECT * FROM vbantraa WHERE TIPOVOUC LIKE '%${testwork}%' AND FECHA BETWEEN '${dateOne}' AND '${dateTwo}'`);
      ...
      const columns = await this.seconddataSource.query(`SHOW COLUMNS FROM vbantraa`);
      ...
      for (const row of dataToTransfer) {
        ...
        const existingRow = await this.seconddataSource.query('SELECT 1 FROM vbantraa WHERE CODIBANC = ? AND TIPOVOUC = ? AND NUMEVOUC = ?',[codibanc, tipovouc, numevouc]);
        ...
        if (existingRow.length === 0) {
          ...
          if (foreignKeyExists.length > 0) {
            const insertQuery = `INSERT INTO vbantraa (${columnNames.join(', ')}) VALUES (${columnNames.map(() => '?').join(', ')})`;
